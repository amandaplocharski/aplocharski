...
These next few questions will deal with grouping. We will first take a look at how many members in the dataset did not favor impeachment come from districts with a GDP that is above the national figure versus below. We will use the "group_by" function to aggregate the "for_impeachment" column from the dataset. Then we are going to usee the "filter" function to say that in that aggregated "for_impeachment" column, we only want data from people who voted no for impeachment, and then those with a GDP above national. The same exact steps are followed in the code below it, only this time, filtering for a GDP below national.
```{r}
